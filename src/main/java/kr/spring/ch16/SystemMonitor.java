 ...
 * @Autowired 어노테이션 사용
 * 생성자, 필드, 메서드에 지정 가능
 * 메서드에 지정할 때는 setXXX뿐만 아니라 일반 메서드에도 적용 가능
 * 
 * @Autowired 어노테이션은 타입을 이용해서 자동적으로 프로퍼티 값을 설정하기 때문에, 해당 타입의 빈 객체가 존재하지 않거나
 * 또는 빈 객체가 두 개 이상 존재할 경우 스프링은 @Autowired 어노테이션이 적용된 빈 객체를 생성할 때 예외를 발생
 * 
...
public class SystemMonitor {
	...
	@Autowired //set메서드 생성후 @Autowired 임포트 -> (required = false) 입력하면 null 로 나옴
